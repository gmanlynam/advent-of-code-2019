fun main() {
    ...
    val firstWire = wires[0].split(',')
    val secondWire = wires[1].split(',')

    val firstSetOfLines = arrayListOf<Pair<Pair<Int,Int>, Pair<Int,Int>>>()
    var firstLineHead = Pair(0,0)
    var secondLineHead = Pair(0,0)
    firstWire.forEachIndexed { index, command ->
            when {
                command.contains('R') -> {
                    val x = firstWire[index-1]
                    val y = command
                    val movement = command.substring(1, command.length).toInt()
                    firstLineHead = Pair(firstLineHead.first + movement, firstLineHead.second);
                    firstSetOfLines.add(firstLineHead)
                }
                command.contains('L') -> {
                    val movement = command.substring(1, command.length).toInt()
                    firstLineHead = Pair(firstLineHead.first - movement, firstLineHead.second);
                ...
                command.contains('U') -> {
                    val movement = command.substring(1, command.length).toInt()
                    firstLineHead = Pair(firstLineHead.first, firstLineHead.second + movement);
                    firstSetOfLines.add(firstLineHead)
                }
                command.contains('D') -> {
                    val movement = command.substring(1, command.length).toInt()
    ...
    fun getLineFromPair
    secondWire.asSequence().forEach { command ->
        when {
            command.contains('R') -> {
                val movement = command.substring(1, command.length).toInt()
                secondLineHead = Pair(firstLineHead.first + movement, firstLineHead.second);
            }
            command.contains('L') -> {
                val movement = command.substring(1, command.length).toInt()
                secondLineHead = Pair(firstLineHead.first - movement, firstLineHead.second);
            ...
            command.contains('U') -> {
                val movement = command.substring(1, command.length).toInt()
                secondLineHead = Pair(firstLineHead.first, firstLineHead.second + movement);
            }
            command.contains('D') -> {
                val movement = command.substring(1, command.length).toInt()
                secondLineHead = Pair(firstLineHead.first, firstLineHead.second - movement);
